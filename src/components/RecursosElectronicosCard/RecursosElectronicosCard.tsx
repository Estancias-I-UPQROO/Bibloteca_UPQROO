...
export const RecursosElectronicosCard = ({ title, description, image, siteLink }: RecursosElectronicosCardProps) => {
    ...
    return (
        <div className="shadow-md bg-gradient-to-t from-white to-slate-200 rounded-3xl h-fit relative hover:rotate-3 transition-transform" onMouseEnter={() => setBlur(true)} onMouseLeave={() => setBlur(false)}>

            <div className={`${blur ? 'opacity-90' : 'opacity-0'} absolute top-1/2 right-1/2 bg-gradient-to-tl from-gray-900 to-black opacity-90 w-full z-10 
